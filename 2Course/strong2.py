...
def test_forbidden_suff():
	...
	assert is_abelian("aabaabaababa", "aab")
	assert is_abelian("aabaabbaaaba", "aab")
...
def main():
	...
	for R in range(1, 7):
		...
		print("Correct - trie\n")
		for w in sorted(list(correct_trie.difference(trie))):
			if len(w) <= 12:
				print(w + " ")
		
		print("\nTrie - correct\n")
		for w in sorted(list(trie.difference(correct_trie))):
			if len(w) <= 12:
				print(w + " ")
